...
import { translateExpenseCategory, translateIncomeCategory } from '../../../../shared/i18n/categories';
...
export const CategoryGrid = () => {
  ...
  const { data: totals, isLoading, isError } = useGetCategoriesQuery({ month, year, type });

  const toggleType = () => {
    dispatch(setType(type === 'expenses' ? 'income' : 'expenses'));
  };

  // Бекенд вже повертає категорії відсортованими за спаданням суми, тому
  // перша (якщо сума > 0) — найбільша стаття витрат/доходів за період.
  const topCategory = totals && totals[0] && totals[0].amount > 0 ? totals[0].category : null;
  ...
  return (
    <div className={styles.card}>
      <div className={styles.header}>
        ...
        <span className={styles.title}>{t(`calculations.categoryGrid.type.${type}`)}</span>
        <button
          type="button"
          className={styles.arrow}
          onClick={toggleType}
      ...
      {isLoading && <p className={styles.state}>{t('calculations.categoryGrid.loading')}</p>}
      {isError && <p className={styles.state}>{t('calculations.categoryGrid.error')}</p>}
      ...
      {!isLoading && !isError && (
        <div className={styles.grid}>
          {(totals ?? []).map(({ category, amount }) => {
            ...
            return (
              <div key={category} className={clsx(styles.tile, isTop && styles.active)}>
