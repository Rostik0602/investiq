...
// Бэкенд отдаёт { statusCode, message, error, timestamp, path }; message
// бывает строкой (ConflictException/UnauthorizedException) или массивом
// строк (class-validator ValidationPipe) — оба варианта приводим к тексту.
...
export const AuthCard = () => {
  ...
  const handleRegister = async (values: { email: string; password: string }) => {
    try {
      // Временный дефолт: имя берём из email до "@", т.к. на макете поля "Ім'я" нет,
      // а бэкенд требует name при регистрации
